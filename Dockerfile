...
RUN apt-get install gcc g++ cmake git sudo libssl-dev zlib1g-dev -y mysql-server mysql-client libmysqlclient-dev
RUN apt-get clean -y
...
#docker images
#sudo docker run --network="host" -t -i sql_test /bin/bash
#sudo docker run --network="host" sql_test